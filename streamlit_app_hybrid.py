...
def load_data():
    ...
    return scam_aus, state_pop, AGE_ORDER, AUS


scam_aus, state_pop, AGE_ORDER, AUS_STATES = load_data()
...
with tab4:
    ...
    st.caption("A composite score combining report volume, total losses, and average loss. Higher score = stronger policy priority.")
    ...
    risk["risk_score"] = (
        risk["volume_rank"] * 0.30 + risk["loss_rank"] * 0.50 + risk["severity_rank"] * 0.20
    ) * 100
    ...
    if len(risk) > 0:
        ...
        pull_quote(
            f"<b>{top_risk}</b> ranks as the highest composite-risk scam type. "
            f"This score is useful because it balances scale, severity, and frequency — "
            f"not relying on any single metric that could mislead.",
